...
st.markdown("""
    <style>
    /* Cache le menu Streamlit par défaut pour faire "vraie application" */
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    
    /* Design des cartes pour les KPI (Métriques) */
...
@st.cache_resource
def connect_sheets():
    """Établit la connexion sécurisée avec l'API Google Sheets via Service Account."""
...
def delete_last_session():
    """Supprime la toute dernière séance (bulk de lignes) enregistrée en base."""
    try:
        client = connect_sheets()
        sheet = client.open("DB_Dynamic_Hybrid_Coach")
        worksheet = sheet.worksheet("Historique_Realise")
...
def calculer_readiness(sommeil, vfc, fcr, energie):
    """Algorithme de calcul du Readiness Score pondérant VFC, FCR, Sommeil et RPE."""
    if 7.5 <= sommeil <= 9.0:
        score_sommeil = 100
...
if page == "Morning Readiness":
    ...
    # ---> LE BOUTON MANQUANT A ÉTÉ REMIS ICI <---
