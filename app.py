...
if page == "Check-in Matinal":
    ...
    if st.button("Valider mon Check-in", type="primary"):
        ...
        nouvelle_ligne_checkin = [date_du_jour, float(sommeil), int(vfc), int(energie), str(muscles_str)]
...
# ---- PAGE 4 : CRÉATEUR DE PROGRAMME ----
    ...
    with tab1:
        ...
        if est_cardio_wod:
            ...
            with col_a: series = st.number_input("Séries cibles", min_value=1, step=1, value=4)
            with col_b: reps = st.text_input("Reps", value="10")
            with col_c: poids = st.number_input("Poids cible (kg)", min_value=0.0, step=0.5, value=0.0)
